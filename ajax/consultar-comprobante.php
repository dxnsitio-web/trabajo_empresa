		...
		while ($reg = $rspta->fetch_object())
				{
					...
					echo '<tr class="filas"><td></td><td>'.$reg->nombre.'</td><td>'.$reg->serie.'</td><td>'.$reg->unidad_medida.'</td><td>'.$reg->cantidad.'</td><td>'.$valorVentaU.'</td><td>'.$newigv.'</td><td>'.$reg->precio_venta.'</td><td>'.$valorVentaT.'</td><td>'.$reg->subtotal.'</td></tr>';
		...
		$rspta=$venta->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			if($reg->codigotipo_comprobante=='1' || $reg->codigotipo_comprobante=='3'){
 				$url='../reportes/Facturacion-Ticket.php?id=';
 			}else{
