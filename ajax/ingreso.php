...
$idcategoria = strval($idcategoria);
...
switch ($_GET["op"])
{
    case 'guardaryeditar':
      if(empty($idingreso))
      {
        $rspta=$ingreso->insertar($idproveedor,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,$_POST["idarticulo"],$_POST["laboratorio"],$idcategoria,$_POST["lote"],$_POST["cantidad"],$_POST["precio_compra"],$_POST["precio_venta"],$_POST["fecha_vencimiento"]);
    ...
    while ($reg=$rspta->fetch_object()){
      $data[]=array(
        "0"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\',\''.$reg->fecha_vencimiento.'\')"><span class="fa fa-plus"></span></button>',
        "1"=>"".$reg->nombre,
        "2"=>$reg->categoria,
        "3"=>$reg->codigo,
        "4"=>$reg->stock,
