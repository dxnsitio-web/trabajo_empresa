...
Class Consultas
{
  ...
  INNER JOIN detalle_ingreso di ON ultimos.idarticulo = di.idarticulo AND ultimos.ultimodetalle = di.iddetalle_ingreso
  WHERE a.condicion='1'   
  ...
  return ejecutarConsulta($sql);
    ...
    public function sumventasfechausuario($fecha_inicio,$fecha_fin,$idusuario,$codigotipo_comprobante,$codigotipo_pago) {
      ...
      // --
      if ($idusuario == "all") {
        $sql="SELECT IFNULL(sum(v.total_venta),0) AS sumatotalusuario, u.nombre as usuario 
        ...
        $sql="SELECT IFNULL(sum(v.total_venta),0) AS sumatotalusuario, u.nombre as usuario 
        FROM venta v inner join usuario u on v.idusuario=u.idusuario  where v.idusuario='$idusuario' and date(v.fecha_hora)>='$fecha_inicio' and date(v.fecha_hora)<='$fecha_fin' $where_codigotipo_comprobante $where_codigotipo_pago 
        AND v.codigotipo_comprobante in (1,3) and v.estado!='Cotizado' and v.estado!='Anulado' and v.estado!='AnuladoC' ";
      }
