...
function agregarDocumento(idventarelacionado,codigonota,idcliente,cliente,num_documento,serie,correlativo,idmoneda,descripcionmoneda,descripcion_tipo_comprobante,fecha,op_gravadas,op_exoneradas,venta_total,impuesto,ultimo_correlativo,serie_correlativo){
	...
	console.log(ultimo_correlativo)
	var tiponotacred;
	if(codigonota=='1'){
	...
	var conceptos=  
        		    ...
        		    '<div class="form-group col-lg-2 col-md-2 col-sm-2 col-xs-12">'+
                            '<div >'+
                            '<input type="text" class="form-control" maxlength="8" name="correlativo" id="correlativo"  value="'+ultimo_correlativo+'" placeholder="correlativo" readonly="readonly" required>'+
                            '</div>'+
        		    '</div>'+
...
function modificarSubtotales()
  {
	...
	$('#totalg').html("S/. " + totalgravad.toFixed(2));
    ...
    $('#totaligv').html("S/. " + totaligv.toFixed(2));
